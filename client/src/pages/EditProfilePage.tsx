...
function getPreviewLabel(profile: OwnProfile | null, displayName: string): string {
  const source =
    displayName.trim() || profile?.displayName?.trim() || profile?.username;
  ...
  return (
    <section className="panel profile-page">
      <div className="auth-copy">
        <p className="auth-kicker">Profile editor</p>
        <h2>Edit your profile</h2>
        <p>
          Update the details other people see when they visit your profile.
        </p>
      </div>
        ...
        <div className="profile-editor-layout">
          <aside className="profile-editor-preview">
            <div className="profile-editor-preview-card">
              {values.avatarUrl ? (
                <img
                  alt={`Preview avatar for ${values.displayName || profile?.username || "profile"}`}
                ...
                >
                  {getPreviewLabel(profile, values.displayName)}
                </div>
              )}
              <p className="auth-kicker profile-editor-preview-kicker">Live preview</p>
              <h3 className="profile-editor-preview-name">
                {values.displayName.trim() || profile?.displayName || "Display name"}
              </h3>
              <p className="profile-editor-preview-handle">
                @{profile?.username ?? "your_handle"}
              </p>
              <p className="profile-editor-preview-bio">
                {values.bio.trim() ||
                  profile?.bio ||
                  "A short bio helps the profile card feel intentional and complete."}
              </p>
            </div>
          </aside>
          ...
          <form className="auth-form profile-form" noValidate onSubmit={handleSubmit}>
            ...
            <label className="form-field" htmlFor="profile-bio">
              ...
              <textarea
                ...
                name="bio"
                rows={5}
                placeholder="Write a short intro, studio note, or creative focus."
                value={values.bio}
